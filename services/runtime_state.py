...
def _presence_dump(value) -> str:
    return json.dumps(value or {})


def _presence_load(value: str | None):
    if not value:
        return {}
    try:
    ...
    if isinstance(parsed, dict):
        return {str(key): str(status) for key, status in parsed.items() if status}
...
def save_doctor_presence(*, doctor_id: int, status: str, channel: str | None = None):
    ...
    with get_connection() as conn:
        ...
        if channel_key:
            cursor.execute(
                "SELECT status FROM doctor_runtime_presence WHERE doctor_id = ?",
                (doctor_id,),
            )
            row = cursor.fetchone()
            status_map = _presence_load(row["status"] if row else None)
            status_map[channel_key] = status
            stored_status = _presence_dump(status_map)
        else:
        ...
        cursor.execute(
            ...
            """,
            (doctor_id, stored_status),
        )
        conn.commit()


def remove_doctor_presence(doctor_id: int, channel: str | None = None):
    with get_connection() as conn:
        cursor = conn.cursor()
        channel_key = (channel or "").strip().lower()
        if channel_key:
            ...
            if not row:
                return
            status_map = _presence_load(row["status"])
            status_map.pop(channel_key, None)
            if status_map:
                cursor.execute(
                    "UPDATE doctor_runtime_presence SET status = ? WHERE doctor_id = ?",
                    (_presence_dump(status_map), doctor_id),
                )
            else:
                cursor.execute(
