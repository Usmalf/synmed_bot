...
class TestConsultationFlow(IsolatedAsyncioTestCase):
    ...
    async def test_doctor_ended_web_consultation_remains_available_for_patient_review(self):
        patient = register_patient(
            telegram_id=101,
            name="Ada",
            age="29",
        ...
        self.assertEqual(status["consultation_id"], consultation_id)
        self.assertEqual(status["doctor"]["doctor_id"], 202)
        self.assertFalse(active_chats)
