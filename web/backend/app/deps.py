...
def require_admin_or_customer_care(session: dict = Depends(get_current_session)) -> dict:
    if session.get("role") not in {"admin", "customer_care"}:
        raise HTTPException(status_code=403, detail="Customer care access is required.")
    return session
