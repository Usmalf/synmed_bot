...
from services.consultation_records import (
    get_consultation_for_feedback,
    get_latest_consultation_for_feedback,
)
...
def rating_keyboard(consultation_id: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [[
            InlineKeyboardButton(f"⭐ {value}", callback_data=f"rate:{consultation_id}:{value}")
...
async def rate_doctor(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    consultation_id_from_button = parts[1] if len(parts) == 3 else ""
    consultation = (
        get_consultation_for_feedback(consultation_id_from_button, patient_id)
        if consultation_id_from_button
        else get_latest_consultation_for_feedback(patient_id)
    )
