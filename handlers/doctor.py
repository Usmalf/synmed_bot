...
from handlers.rate_doctor import rating_keyboard
...
async def doctor_on(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if registry.is_doctor_available(doctor_id, "telegram"):
        await update.message.reply_text(
    ...
    patient_id, patient_details = registry.pop_waiting_patient("telegram")
    if patient_id is None:
        registry.set_doctor_available(doctor_id, channel="telegram")
        await update.message.reply_text(
...
async def doctor_off(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    restore_runtime_state()
    registry.restore_runtime_state()
    registry.remove_doctor_from_runtime(doctor_id, channel="telegram")
    ...
    if patient_details.get("source") != "web":
        await context.bot.send_message(
            ...
            reply_markup=rating_keyboard(consultation["consultation_id"]),
        )
    await update.message.reply_text(
